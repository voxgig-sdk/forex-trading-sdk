...
[MCP](https://modelcontextprotocol.io) server exposing the ForexTrading SDK as
two agent tools — `forex-trading_list` and `forex-trading_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/forex-trading-mcp)
make build

# 2. Provide credentials via the environment
export FOREX_TRADING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user forex-trading \
  -- /absolute/path/to/forex-trading-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./forex-trading-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// forex-trading_list: first page of records
{ "entity": "market_data" }
{ "entity": "market_data", "query": { } }

// forex-trading_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/forex-trading-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user forex-trading \
     -- "$PWD"/dist/*/forex-trading-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `forex-trading_list` and `forex-trading_load` tools now appear
   in new sessions. Ask the agent to *"list market_data using forex-trading"*
   and it calls `forex-trading_list` with `{"entity":"market_data"}`.
...
```sh
export FOREX_TRADING_APIKEY=sk_live_xxx            # API key
export FOREX_TRADING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `forex-trading_list` tool
...
### Call the `forex-trading_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `forex-trading_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `forex-trading_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FOREX_TRADING_APIKEY` | API key sent with every request. |
| `FOREX_TRADING_BASE` | Optional override of the API base URL. |
...
```sh
./forex-trading-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"forex-trading_load","arguments":{"entity":"market_data","query":{"id":1}}}}'
```
