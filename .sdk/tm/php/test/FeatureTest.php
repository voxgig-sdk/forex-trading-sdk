...
class FtCtrl extends ForexTradingControl
{
...
class FtClient
{
    ...
    private ForexTradingUtility $utility;

    public function __construct(ForexTradingUtility $utility, array $options)
    {
    ...
    public function get_utility(): ForexTradingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ForexTradingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ForexTradingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ForexTradingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ForexTradingConfig::make_feature($name);
    ...
    private static function buildUrl(ForexTradingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ForexTradingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ForexTradingResponse($fetched);
        }
    ...
    private function fail(ForexTradingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ForexTradingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
