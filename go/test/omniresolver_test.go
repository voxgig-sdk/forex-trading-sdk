...
import (
	...
	omni "github.com/voxgig-sdk/forex-trading-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ForexTradingError); is {
				out := map[string]any{
					"name":    "ForexTradingError",
...
func omniCtx(arg any, client *sdk.ForexTradingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ForexTradingSDK); is {
			client = live
