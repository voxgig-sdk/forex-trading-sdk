// Command ForexTrading-cli is a CLI + REPL for the ForexTrading SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      FOREX_TRADING_APIKEY (api key), FOREX_TRADING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: FOREX_TRADING_APIKEY carries the API key and
	// FOREX_TRADING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("FOREX_TRADING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("FOREX_TRADING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewForexTradingSDK(opts)
