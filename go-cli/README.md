...
boru-driven command-line client **and** interactive REPL for the ForexTrading
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/forex-trading-cli)
make build

# 2. See usage (words, entities, env vars)
./forex-trading-cli --help
...
FOREX_TRADING_BASE=https://api.example.com ./forex-trading-cli list market_data

# 6. No arguments -> interactive REPL
./forex-trading-cli
forex-trading> list market_data
forex-trading> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/forex-trading-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/forex-trading-cli list market_data
   ```
...
Run with no arguments to open a REPL (prompt `forex-trading>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/forex-trading-cli`. |
...
(`./forex-trading-cli <expr>`) and interactively (the REPL), and expressions compose the
