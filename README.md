...
The Forex Trading API allows users to access the foreign exchange market for trading currencies and precious metals.

Learn more about Forex Trading API at [swissquote.com](https://swissquote.com).
...
const items = await client.MarketData().list()
```
...
```go
client := sdk.Test()
result, err := client.MarketData(nil).List(
    nil, nil,
)
```
...
$marketdatas = $client->MarketData()->list();
print_r(array_map(fn($item) => $item->data_get(), $marketdatas));
```
