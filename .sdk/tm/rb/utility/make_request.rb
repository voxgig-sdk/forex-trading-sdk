...
module ForexTradingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ForexTradingResponse.new({})
    ...
    if fetch_err
      ...
      response = ForexTradingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ForexTradingResponse.new(fetched)
    else
