...
// ForexTrading SDK retry feature
...
class ForexTradingRetryFeature extends ForexTradingBaseFeature
{
    ...
    public function init(ForexTradingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ForexTradingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ForexTradingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ForexTradingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
