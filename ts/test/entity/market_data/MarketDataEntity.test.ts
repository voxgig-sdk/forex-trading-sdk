...
import { ForexTradingSDK, BaseFeature, stdutil } from '../../..'
...
describe('MarketDataEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set FOREX_TRADING_TEST_MARKET_DATA_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  let client = ForexTradingSDK.test(options, extra)
  ...
  const idmapEnvVal = process.env['FOREX_TRADING_TEST_MARKET_DATA_ENTID']
