# ForexTrading PHP SDK



The PHP SDK for the ForexTrading API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->MarketData()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $marketdata));
```
...
```php
$client = ForexTradingSDK::test($testopts, $sdkopts);
```
...
```
php/
├── forextrading_sdk.php          -- Main SDK class
...
The main class (`forextrading_sdk.php`) exports the SDK class
