...
//     (\`new ForexTradingSDK(...)\` / \`ForexTradingSDK.test(...)\` become
//     \`ForexTradingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ForexTradingSDK'
...
// rewrite real client construction (\`new ForexTradingSDK(...)\` and any
// \`ForexTradingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ForexTradingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ForexTradingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
