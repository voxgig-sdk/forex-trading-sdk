...
require_relative 'ForexTrading_types'


class ForexTradingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ForexTradingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ForexTradingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ForexTradingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ForexTradingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ForexTradingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ForexTradingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ForexTradingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ForexTradingError.new(
        "#{op}_allow",
        "ForexTradingSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ForexTradingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ForexTradingError.new(
        "graphql_error", "ForexTradingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ForexTradingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
